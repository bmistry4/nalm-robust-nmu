...
plot_acc_vs_ep = function(plot_data, y_axis_label) {
    ...
    return(
        plot_data %>%
          ...
          # TODO - remove ribbon if too fluctuant
...
summary_dat_test_output = data_summary(expanded_dat, quo(metric.test.output.loss), 'output (test)')
summary_dat_train_output = data_summary(expanded_dat, quo(metric.train.output.loss), 'output (train)')
summary_dat_valid_output = data_summary(expanded_dat, quo(metric.valid.output.loss), 'output (valid)')

summary_dat_test_lab1 = data_summary(expanded_dat, quo(metric.test.label1.loss), 'label 1 (test)')
summary_dat_train_lab1 = data_summary(expanded_dat, quo(metric.train.label1.loss), 'label 1 (train)')
summary_dat_valid_lab1 = data_summary(expanded_dat, quo(metric.valid.label1.loss), 'label 1 (valid)')

summary_dat_test_lab2 = data_summary(expanded_dat, quo(metric.test.label2.loss), 'label 2 (test)')
summary_dat_train_lab2 = data_summary(expanded_dat, quo(metric.train.label2.loss), 'label 2 (train)')
summary_dat_valid_lab2 = data_summary(expanded_dat, quo(metric.valid.label2.loss), 'label 2 (valid)')

# plot the summary stats for each model
summary_dat = rbind(summary_dat_train_output, summary_dat_valid_output, summary_dat_test_output,
                    summary_dat_train_lab1, summary_dat_valid_output, summary_dat_test_lab1,
                    summary_dat_train_lab2, summary_dat_valid_output, summary_dat_test_lab2)
print(summary_dat)
ci_plot = plot_scatter_with_se(summary_dat, y_axis_label = 'Loss')
#ci_plot

# save pdf and the the data to generate the pdf.
ggsave(paste0(name.output, '_loss', '.pdf'), ci_plot, device = "pdf")
write.csv(summary_dat, paste0(name.output, '_loss', '_plot_data.csv'))
...
summary_dat_valid_output = data_summary(expanded_dat, quo(metric.valid.output.acc), 'output (valid)')
...
summary_dat_valid_lab1 = data_summary(expanded_dat, quo(metric.valid.label1.acc), 'label 1 (valid)')
...
summary_dat_valid_lab2 = data_summary(expanded_dat, quo(metric.train.label2.acc), 'label 2 (valid)')

# plot the summary stats for each model
#summary_dat = rbind(summary_dat_train_output, summary_dat_test_output, summary_dat_valid_output,
#                    summary_dat_train_lab1, summary_dat_test_lab1, summary_dat_valid_lab1,
#                    summary_dat_train_lab2, summary_dat_test_lab2, summary_dat_valid_lab2)
...
########################################################################################################################
### PLOTTING ROUNDED ACCURACY WITH ERROR BARS
# load the data
expanded_dat = load_and_merge_csvs(merge_mode, single_filepath = paste0(load_folder, base_filename, '.csv'))
# filter to single epoch so each fold for each model has 1 result (row of data)
expanded_dat = filter_by_step(expanded_dat, step_value = FILTER_EPOCH)
...
# summarise the test and train data
summary_dat_test_output = data_summary(expanded_dat, quo(metric.test.output_rounded.acc), 'output (test)')
summary_dat_train_output = data_summary(expanded_dat, quo(metric.train.output_rounded.acc), 'output (train)')
summary_dat_valid_output = data_summary(expanded_dat, quo(metric.valid.output_rounded.acc), 'output (valid)')

summary_dat_test_lab1 = data_summary(expanded_dat, quo(metric.test.label1_rounded.acc), 'label 1 (test)')
summary_dat_train_lab1 = data_summary(expanded_dat, quo(metric.train.label1_rounded.acc), 'label 1 (train)')
summary_dat_valid_lab1 = data_summary(expanded_dat, quo(metric.valid.label1_rounded.acc), 'label 1 (valid)')

summary_dat_test_lab2 = data_summary(expanded_dat, quo(metric.test.label2_rounded.acc), 'label 2 (test)')
summary_dat_train_lab2 = data_summary(expanded_dat, quo(metric.train.label2_rounded.acc), 'label 2 (train)')
summary_dat_valid_lab2 = data_summary(expanded_dat, quo(metric.train.label2_rounded.acc), 'label 2 (valid)')
...
# save pdf and the the data to generate the pdf.
ggsave(paste0(name.output, '_rounded_acc', '.pdf'), ci_plot, device = "pdf")
write.csv(summary_dat, paste0(name.output, '_rounded_acc', '_plot_data.csv'))
########################################################################################################################
...
########################################################################################################################
### PLOTTING ACCURACY VS EPOCHS FOR ROUNDED ACCS
# load the data
expanded_dat = load_and_merge_csvs(merge_mode, single_filepath = paste0(load_folder, base_filename, '.csv'))
expanded_dat = expanded_dat %>% group_by(model, epoch)
...
# save pdf and the the data to generate the pdf.
#ggsave(paste0(name.output, '_ep-acc_NO-RIBBON', '.pdf'), epoch_plot, device = "pdf")
ggsave(paste0(name.output, '_ep-rounded_acc', '.pdf'), epoch_plot, device = "pdf")
write.csv(summary_dat_test_output, paste0(name.output, '_ep-rounded_acc', '_plot_data.csv'))

# plot the summary stats for each model
epoch_plot = plot_acc_vs_ep(summary_dat_test_lab1, 'Left Label Accuracy (%)')
# save pdf and the the data to generate the pdf.
ggsave(paste0(name.output, '_ep-rounded_acc-lab1', '.pdf'), epoch_plot, device = "pdf")
write.csv(summary_dat_test_lab1, paste0(name.output, '_ep-rounded_acc-lab1', '_plot_data.csv'))

# plot the summary stats for each model
epoch_plot = plot_acc_vs_ep(summary_dat_test_lab2, 'Right Label Accuracy (%)')
# save pdf and the the data to generate the pdf.
ggsave(paste0(name.output, '_ep-rounded_acc-lab2', '.pdf'), epoch_plot, device = "pdf")
write.csv(summary_dat_test_lab2, paste0(name.output, '_ep-rounded_acc-lab2', '_plot_data.csv'))
