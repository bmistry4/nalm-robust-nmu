...
dataset.get.distribution = function (distribution) {
  return(substring(distribution, 3))

}

convert.last.char.to.bool = function (code) {
  # e.g. TRUE for L1T and FALSE for L1F
  return(substring(code,3) == 'T')
}

beta.get.part = function(beta, position) {
  # only works for beta start and end values which use standard math notation (i.e. 1e-XX) for values <1 and that you
  # do not have a case of the end value being 1e-X and the start value being a number in non-standard notation.

  # e.g. "rb"    "1e"    "11"    "1e"    "08"    "10000" "10"
  split = strsplit(beta, '-')[[1]]
  if (length(split) == 5) {
    # neither start nor end has a 1e-XX
    start = as.numeric(split[2])
    end = as.numeric(split[3])

  } else if (length(split) == 6) {
    # only start has a 1e-XX
    # e.g. '1e' + '-' + '08' = 1e-08
    start = as.numeric(paste0(strsplit(beta, '-')[[1]][2], '-', strsplit(beta, '-')[[1]][3]))
    end = as.numeric(split[4])

  } else if (length(split) == 7) {
    # both start and end have 1e-XX
    start = as.numeric(paste0(strsplit(beta, '-')[[1]][2], '-', strsplit(beta, '-')[[1]][3]))
    end = as.numeric(paste0(strsplit(beta, '-')[[1]][4], '-', strsplit(beta, '-')[[1]][5]))
  }
...
npu.W.get.part = function(reg, index) {
  split = strsplit(reg, '-')[[1]]
  val = split[index+1]
  if(index == 1){
...
expand.name = function (df) {
  ...
  df.expand.name = names %>%
    ...
    mutate(
      ...
      train.distribution = dataset.get.distribution(extract.by.split(name, 16)),
      nmu.noise = convert.last.char.to.bool(extract.by.split(name, 25)),  # note: not in order
      noise.range = ifelse(nmu.noise == TRUE, range.full.to.short(extract.by.split(name, 17)), NA),

      l1 = convert.last.char.to.bool(extract.by.split(name, 18)),
      regualizer.beta.start = beta.get.part(extract.by.split(name, 19), 'start'),
      regualizer.beta.end = beta.get.part(extract.by.split(name, 19), 'end'),
      # index 19 -> ignore step and growth because remain constant for all experiements
      regualizer.npu.W = npu.W.get.part(extract.by.split(name, 20), 1),
      regualizer.npu.W.type = npu.W.get.part(extract.by.split(name, 20), 2),
      regualizer.gate = as.numeric(strsplit(extract.by.split(name, 21), '-')[[1]][2]),

      # index 22 -> ignore npu reg type because it's always Madsen ('M')
      #regualizer.scale.type = ifelse(substring(extract.by.split(name, 22),2) == 'M', 'madsen', 'heim'),
      npu.clip = strsplit(extract.by.split(name, 23), '-')[[1]][2],
      npu.W_real.init = strsplit(extract.by.split(name, 24), '-')[[1]][2],

      grad.norm.clip = ifelse(strsplit(extract.by.split(name, 26), '-')[[1]][2] == "F",
                              NA, as.numeric(strsplit(extract.by.split(name, 26), '-')[[1]][2])),

      log.interval = as.integer(strsplit(extract.by.split(name, 27), '-')[[1]][2]),
      # index 28 -> ignore train loss because always mse for robustness exps
      #train.loss = extract.by.split(name, 28)
    )
...
#dat = read.csv('C:\\Users\\mistr\\Documents\\SOTON\\PhD\\Code\\nalu-stable-exp\\export\\single_layer_task\\robustness\\realnpu-mod_uniform--2--1.csv')
#row_1 = dat[1,]$name
#beta.get.part(extract.by.split(dat[159990,]$name, 19),'start')
#beta.get.part(extract.by.split(dat[159990,]$name, 19),'end')