...
load.csv.merge.eps = function(lookup.name) {
  return(switch(
    ...
    "nips-divBy0-easy" = csv.and.eps.merger(list(
      paste(load_folder, 'easy_realnpu_L1F_WG-clip_WrI-xuc', csv_ext, sep = ''),
      paste(load_folder, 'reciprocal_in-1_nru-tanh1000_lr-1', csv_ext, sep = ''),
      paste(load_folder, 'easy_signNMRU-correctEps-gnc-1_lr-1e-2_E5K', csv_ext, sep = '')
    ),
    ...
    "nips-divBy0-medium" = csv.and.eps.merger(list(
      paste(load_folder, 'medium_realnpu_clip-WG_M-S40K-50K-G1-W1_WrI-xuc', csv_ext, sep = ''),
      paste(load_folder, 'reciprocal_in-2_nru-tanh1000_lr-1', csv_ext, sep = ''),
      paste(load_folder, 'medium_signNMRU-correctEps-gnc-1_lr-1e-2', csv_ext, sep = '')
    ),
      list('Real NPU', 'NRU', 'NMRU'),
      list('zero.range.medium.realnpu', 'zero.range.medium', 'zero.range.medium.nmru')
    ),
    "nips-divBy0-hard" = csv.and.eps.merger(list(
      paste(load_folder, 'hard_realnpu_clip-WG_M-S40K-50K-G1-W1_WrI-xuc', csv_ext, sep = ''),
      paste(load_folder, 'in-2_nru-tanh1000_lr-1', csv_ext, sep = ''),
      paste(load_folder, 'hard_signNMRU-correctEps-gnc-1_lr-1e-2', csv_ext, sep = '')
    ),
      list('Real NPU', 'NRU', 'NMRU'),
